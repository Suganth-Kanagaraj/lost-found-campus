...
class Item(db.Model):
    ...
    category = db.Column(db.String(50))  # lost, found
    item_type = db.Column(db.String(100))  # electronics, books, wallet, etc.
    location = db.Column(db.String(200))
    campus_area = db.Column(db.String(100))  # library, cafeteria, etc.
    image_hash = db.Column(db.String(100))  # For image similarity
    image_path = db.Column(db.String(300))
    status = db.Column(db.String(20), default='pending')  # pending, matched, resolved
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Match(db.Model):
    ...
    status = db.Column(db.String(20), default='pending')  # pending, viewed, accepted
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def find_matches(item_id, item_hash):
    """Find similar items using image matching"""
    if not item_hash:
        return []
    ...
    for item in all_items:
        if item.category == item.category:  # Same type (lost/found)
            similarity = calculate_similarity(item_hash, item.hash)
...
def dashboard():
    ...
    return render_template('dashboard.html', user=user, my_items=my_items, 
...
def report_item():
    ...
    if request.method == 'POST':
        ...
        category = request.form.get('category')  # lost or found
        ...
        if image_hash:
            ...
            for match in matches:
                new_match = Match(
                    found_item_id=match['item'].id if match['item'].category == 'found' else new_item.id,
                    lost_item_id=new_item.id if match['item'].category == 'found' else match['item'].id,
...
def item_detail(item_id):
    ...
    matches = Match.query.filter(
        ((Match.found_item_id == item_id) | (Match.lost_item_id == item_id))
    ).all()
...
def api_items():
    ...
    return jsonify([{
        ...
        'image': url_for('static', filename='uploads/' + item.image_path) if item.image_path else None,
